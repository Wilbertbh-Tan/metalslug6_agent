...
def main():
    parser = argparse.ArgumentParser(
        description="RAM-based reward monitor for Metal Slug 6"
    )
    parser.add_argument("--host", default="127.0.0.1", help="RetroArch host")
    parser.add_argument(
        "--port", type=int, default=55355, help="RetroArch UDP command port"
    )
    parser.add_argument(
        "--poll-hz", type=float, default=30, help="Polling frequency in Hz"
    )
    parser.add_argument(
        "--reset-delay",
        type=float,
    ...
    parser.add_argument(
        "--game-state-addr", default="003868D0", help="RAM address for game_state byte"
    )
    parser.add_argument(
        "--lives-addr",
        default="003D3B07",
        help="RAM address for lives byte (actual counter)",
    )
    parser.add_argument(
        "--bomb-addr", default="003D3F45", help="RAM address for bombs byte"
    )
    parser.add_argument(
        "--arms-addr", default="003D3F48", help="RAM address for arms/ammo (2 bytes LE)"
    )
    parser.add_argument(
        "--time-addr", default="003FB939", help="RAM address for in-game timer (BCD)"
    )
    parser.add_argument(
        "--score-addr", default="003869BC", help="RAM address for 4-byte BCD score"
    )
    ...
    print(
        f"  bomb_addr={args.bomb_addr}  arms_addr={args.arms_addr}  time_addr={args.time_addr}"
    )
    ...
    try:
        while True:
            ...
            if lives_changed:
                gs_str = (
                    f"gs=0x{stable_gs.confirmed:02X}"
                    if stable_gs.confirmed is not None
                    else ""
                )
                print(
                    f"  [{elapsed:7.1f}s] lives -> {stable_lives.confirmed}  {gs_str}"
                )

            if gs_changed:
                gs_hex = (
                    f"0x{stable_gs.confirmed:02X}"
                    if stable_gs.confirmed is not None
                    else "??"
                )
            ...
            if score is not None:
                ...
                if 0 < score_delta < 50000:
                    print(
                        f"  [{elapsed:7.1f}s] SCORE +{score_delta} (total: {score:,})"
                    )
                prev_score = score
            ...
            if (
                lives_seen_positive
                and stable_lives.confirmed is not None
                and stable_lives.confirmed == 0
            ):
                episode += 1
